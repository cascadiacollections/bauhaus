"""Tests for stylize.py — _load_image resolution handling and alpha mask generation."""

import torch
from PIL import Image

from stylize import _load_image, gradient_alpha_mask, luminance_alpha_mask


class TestLoadImage:
    def test_default_max_size_is_1920(self):
        """Default max_size should be 1920."""
        img = Image.new("RGB", (3000, 2000))
        tensor = _load_image(img)
        # Longest side should be scaled to 1920
    ...
    def test_no_upscale(self):
        ...
        tensor = _load_image(img, max_size=1920)
        _, _, h, w = tensor.shape
        assert w == 400
        assert h == 300
